...
still be referred as "tenant" in fic.
...
*/
func AuthOptionsFromEnv() (fic.AuthOptions, error) {
	...
	if authURL == "" {
		err := fic.ErrMissingEnvironmentVariable{
	...
	if username == "" && userID == "" {
		err := fic.ErrMissingAnyoneOfEnvironmentVariables{
