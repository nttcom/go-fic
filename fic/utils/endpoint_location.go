...
*/
func V3EndpointURL(catalog *tokens3.ServiceCatalog, opts fic.EndpointOpts) (string, error) {
	...
	for _, entry := range catalog.Entries {
		if (entry.Type == opts.Type) && (opts.Name == "" || entry.Name == opts.Name) {
			for _, endpoint := range entry.Endpoints {
				if opts.Availability != fic.AvailabilityPublic {
					err := &ErrInvalidAvailabilityProvided{}
					err.Argument = "Availability"
					err.Value = opts.Availability
					return "", err
				}
				if (opts.Availability == fic.Availability(endpoint.Interface)) &&
	...
	for _, endpoint := range endpoints {
		return fic.NormalizeURL(endpoint.URL), nil
	}

	// Report an error if there were no matching endpoints.
	err := &fic.ErrEndpointNotFound{}
	return "", err
}
