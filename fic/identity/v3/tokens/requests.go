...
type AuthOptions struct {
	...
	// AllowReauth should be set to true if you grant permission for GO-FIC
	// to cache your credentials in memory, and to allow GO-FIC to attempt
...
func (opts *AuthOptions) ToTokenV3ScopeMap() (map[string]interface{}, error) {
	...
	return ficAuthOpts.ToTokenV3ScopeMap()
}
...
func subjectTokenHeaders(c *fic.ServiceClient, subjectToken string) map[string]string {
...
func Create(c *fic.ServiceClient, opts AuthOptionsBuilder) (r CreateResult) {
	...
	resp, err := c.Post(tokenURL(c), b, &r.Body, &fic.RequestOpts{
...
func Get(c *fic.ServiceClient, token string) (r GetResult) {
	resp, err := c.Get(tokenURL(c), &r.Body, &fic.RequestOpts{
...
func Validate(c *fic.ServiceClient, token string) (bool, error) {
	resp, err := c.Head(tokenURL(c), &fic.RequestOpts{
		MoreHeaders: subjectTokenHeaders(c, token),
...
func Revoke(c *fic.ServiceClient, token string) (r RevokeResult) {
	_, r.Err = c.Delete(tokenURL(c), &fic.RequestOpts{
		MoreHeaders: subjectTokenHeaders(c, token),
	})
	return
}
