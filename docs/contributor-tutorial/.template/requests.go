...
func List(client *fic.ServiceClient, opts ListOptsBuilder) pagination.Pager {
	url := listURL(client)
	if opts != nil {
...
func Get(client *fic.ServiceClient, id string) (r GetResult) {
...
func (opts CreateOpts) ToResourceCreateMap() (map[string]interface{}, error) {
	return fic.BuildRequestBody(opts, "resource")
}

// Create creates a new RESOURCE.
func Create(client *fic.ServiceClient, opts CreateOptsBuilder) (r CreateResult) {
	b, err := opts.ToResourceCreateMap()
	if err != nil {
		r.Err = err
		return
	}
	_, r.Err = client.Post(createURL(client), &b, &r.Body, &fic.RequestOpts{
		OkCodes: []int{201},
	})
	return
}
...
func Delete(client *fic.ServiceClient, id string) (r DeleteResult) {
...
func Update(client *fic.ServiceClient, id string, opts UpdateOptsBuilder) (r UpdateResult) {
	b, err := opts.ToResourceUpdateMap()
	if err != nil {
		r.Err = err
		return
	}
	_, r.Err = client.Patch(updateURL(client, id), b, &r.Body, &fic.RequestOpts{
		OkCodes: []int{200},
	})
	return
}
