...
`fic.ProviderClient.HTTPClient` field):

```go
//...

// LogRoundTripper satisfies the http.RoundTripper interface and is used to
// customize the default GO-FIC RoundTripper to allow for logging.
...
Extensions that are supported in GO-FIC can be combined to create a custom object:
...
func (r *MyVolume) UnmarshalJSON(b []byte) error {
	...
	var s struct {
		tmp
		TimeCreated fic.JSONRFC3339MilliNoZ `json:"created_at"`
	}
