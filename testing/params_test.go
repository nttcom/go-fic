...
func TestMaybeString(t *testing.T) {
	testString := ""
	var expected *string
	actual := fic.MaybeString(testString)
...
func TestBuildQueryString(t *testing.T) {
	...
	actual, err := fic.BuildQueryString(&opts)
	if err != nil {
	...
	_, err = fic.BuildQueryString(&opts)
	if err == nil {
		t.Errorf("Expected error: 'Required field not set'")
	}
	th.CheckDeepEquals(t, expected, actual)

	_, err = fic.BuildQueryString(map[string]interface{}{"Number": 4})
	if err == nil {
...
func TestBuildHeaders(t *testing.T) {
	...
	actual, err := fic.BuildHeaders(&testStruct)
	th.CheckNoErr(t, err)
	th.CheckDeepEquals(t, expected, actual)

	testStruct.Num = 0
	_, err = fic.BuildHeaders(&testStruct)
	if err == nil {
		t.Errorf("Expected error: 'Required header not set'")
	}

	_, err = fic.BuildHeaders(map[string]interface{}{"Number": 4})
	if err == nil {
		t.Errorf("Expected error: 'Options type is not a struct'")
	}
}
...
func TestQueriesAreEscaped(t *testing.T) {
	...
	actual, err := fic.BuildQueryString(foo{Name: "blah+?!!foo", Shape: "Triangl e"})
...
func TestBuildRequestBody(t *testing.T) {
	...
	// AuthOptions wraps a GO-FIC AuthOptions in order to adhere to the AuthOptionsBuilder
	// interface.
	...
	for _, successCase := range successCases {
		actual, err := fic.BuildRequestBody(successCase.opts, "auth")
		th.AssertNoErr(t, err)
	...
	for _, failCase := range failCases {
		_, err := fic.BuildRequestBody(failCase.opts, "auth")
	...
	actual, err := fic.BuildRequestBody(complexFields, "")
	th.AssertNoErr(t, err)
