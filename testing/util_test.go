...
func TestWaitFor(t *testing.T) {
	err := fic.WaitFor(2, func() (bool, error) {
		return true, nil
	})
	th.CheckNoErr(t, err)
}
...
func TestWaitForTimeout(t *testing.T) {
	...
	err := fic.WaitFor(1, func() (bool, error) {
		return false, nil
	})
...
func TestNormalizeURL(t *testing.T) {
	...
	for i := 0; i < len(expected); i++ {
		th.CheckEquals(t, expected[i], fic.NormalizeURL(urls[i]))
	}
...
func TestNormalizePathURL(t *testing.T) {
	...
	result, _ := fic.NormalizePathURL(basePath, rawPath)
